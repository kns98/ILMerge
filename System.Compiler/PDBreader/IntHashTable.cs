    ...
    internal class IntHashTable
    {
        ...
        */
        ...
        internal IntHashTable(int capacity, int loadFactorPerc)
        {
            ...
            var hashsize = GetPrime(capacity / this.loadFactorPerc);
        ...
        internal object this[int key]
        {
            get
            {
                if (key < 0) throw new ArgumentException("Argument_KeyLessThanZero");
                uint seed;
                uint incr;
                ...
                do
                {
                    ...
                    if ((b.hash_coll & 0x7FFFFFFF) == hashcode && key == b.key) return b.val;
                    seed += incr;
                } while (b.hash_coll < 0 && ++ntry < lbuckets.Length);
        ...
        {
            if (minSize < 0) throw new ArgumentException("Arg_HTCapacityOverflow");
        ...
        private void rehash(int newsize)
        {
            ...
            {
                var oldb = buckets[nb];
                if (oldb.val != null) putEntry(newBuckets, oldb.key, oldb.val, oldb.hash_coll & 0x7FFFFFFF);
            }
            ...
            if (loadsize >= newsize) loadsize = newsize - 1;
        }
        ...
        {
            if (key < 0) throw new ArgumentException("Argument_KeyLessThanZero");
            if (nvalue == null) throw new ArgumentNullException("nvalue", "ArgumentNull_Value");
            if (count >= loadsize)
                expand();
            else if (occupancy > loadsize && count > 100) rehash();
            ...
            var ntry = 0;
            var emptySlotNumber =
                -1; // We use the empty slot number to cache the first empty slot. We chose to reuse slots
            ...
            do
            {
                ...
                {
                    if (add) throw new ArgumentException("Argument_AddingDuplicate__" + buckets[bucketNumber].key);
                    buckets[bucketNumber].val = nvalue;
                    version++;
                    return;
                }
